function ArrowMain600mm(subject,serialNum)
% 目标刺激为→，掩蔽刺激为分割的noise和中文的方位词“上下左右”，两个condition放在一起做
...
try
    ...
    % 定义刺激大小----------------------------
    textSize = 24;       % 文本刺激的大小
    textSizeTargetChi = 23; % 中文掩蔽刺激的大小
    ...
    fid = fopen(['Results\',currentTime,'ArrowMain600mm-',subject,'.txt'],'w');
    fprintf(fid,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','Sub','ISI','MaskType','ReportType','CueDelay','RowCued','TargetOriginal','DigitWhole','Mask','Response','NumCorrect','Accuracy','BackgroundCol','TargetCol','MaskCol','TargetDura','ISIDura','MaskDura','CueDura','ConditioinOrder','BlockOrder','ResponseTime');
    ...
    % 定义实验条件的参数----------------------------
    maskType = {'NoiseMulti','ChiOriens'};  % 掩蔽类型：单个的噪音掩蔽
    ...
    % 不同刺激的微调参数
    ajustArrow = [-4,5];  % 每个箭头所需微调的参数
    ajustNoise = [-5,2];  % 每个noise方块所需微调的参数
    ajustChi = [-4,2];    % 每个中文汉字所需微调的参数
    widthArrow = 25;  % 单个箭头的宽度
    heightArrow = 25; % 单个箭头的高度
    widthNoise = 28;  % 单个noise的宽度
    heightNoise = 30; % 单个noise的高度
    ...
    for indexCondition = 1:2
        for indexBlock = 1:2
            ...
            for t = 1:trialsPart
                ...
                % 写入文件
                fprintf(fid,'%s\t%d\t%s\t%s\t%d\t%d\t%s\t%s\t%s\t%s\t%d\t%6.2f\t%6.4f\t%6.4f\t%6.4f\t%6.6f\t%6.6f\t%6.6f\t%6.6f\t%d%d\t%d%d\t%6.6f\n',...
                        subject,ISI,maskType{orderCondition(indexCondition)},'P',cueDelay(orderBlock(indexBlock)),cuePosition(t),arrowRowOriginal,targetArrowRecord,maskRecord,strRes,numCorrect,accuracy,...
                        backgroundCol(1),targetCol(1),maskCol(1),targetDura,blankDura,maskDura,cueDura,orderCondition(1),orderCondition(2),orderBlock(1),orderBlock(2),responseTime);
                % 一个完整的trial结束------------------------------------------------------
            ...
            % 呈现组间休息结束指导语--------------------------------------
            DrawFormattedText(w , '本组实验结束，请稍事休息。\n\n就绪后按"Enter"听提示音继续', 'center', 'center', targetCol);
            Screen('Flip',w);
            % 释放所有按键，谨防卡住------------------------------------
            keyisdown = 1;
            while(keyisdown) % first wait until all keys are released
        ...
        % 判断condition结束，还是整个实验结束，呈现相应指导语-------------------
        if indexCondition == 2
            % 呈现实验结束指导语---------------------------------------
            DrawFormattedText(w , '本实验已结束，谢谢参与。\n\n请呼叫主试', 'center', 'center', targetCol);
            Screen('Flip',w);
            % 释放所有按键，谨防卡住------------------------------------
            keyisdown = 1;
            while(keyisdown) % first wait until all keys are released
