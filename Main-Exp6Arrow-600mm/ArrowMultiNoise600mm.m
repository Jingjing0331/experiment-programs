function ArrowMultiNoise600mm(subject,serialNum)
% 目标刺激为→，掩蔽刺激为分割的noise
...
try
    ...
    fid = fopen(['Results\',currentTime,'ArrowNoise600mm-',subject,'.txt'],'w');
    fprintf(fid,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','Sub','ISI','MaskType','ReportType','CueDelay','RowCued','RowOriginal','TargetWhole','Mask','Response','NumCorrect','Accuracy','BackgroundCol','TargetCol','MaskCol','TargetDura','ISIDura','MaskDura','CueDura','BlockOrder','ResponseTime');
    ...
    % 定义实验条件的参数----------------------------
    maskType = 'NoiseMulti';  % 掩蔽类型：单个的噪音掩蔽
    ...
    for indexBlock = 1:2
        ...
        for t = 1:trialsPart
            ...
            fprintf(fid,'%s\t%d\t%s\t%s\t%d\t%d\t%s\t%s\t%s\t%s\t%d\t%6.2f\t%6.4f\t%6.4f\t%6.4f\t%6.6f\t%6.6f\t%6.6f\t%6.6f\t%d%d\t%6.6f\n',...
                    subject,ISI,maskType,'P',cueDelay(orderBlock(indexBlock)),cuePosition(t),arrowRowOriginal,targetArrowRecord,'',strRes,numCorrect,accuracy,...
