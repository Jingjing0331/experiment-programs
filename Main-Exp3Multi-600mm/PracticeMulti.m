function PracticeMulti(subject,trialsWhole,trialsPart)
% 掩蔽刺激为多个白噪音
try
    ...
    %打开结果保存文件
    fid = fopen(['Results\','PracticeMulti-',subject,'.txt'],'w');
    ...
    KbWait;
    %呈现Pattern流程图
    patternIns_img = imread('Instructions\Pattern.jpg'); %读取Pattern条件下的知道流程图
    patternIns = Screen('MakeTexture',w,patternIns_img); %将图片转换为Texture
    Screen('DrawTexture',w,patternIns); %呈现Pattern条件下的流程图
    Screen('Flip',w); %将缓存区画好的图片进行呈现
    %释放所有按键，谨防卡住
    keyisdown = 1;
...
for n = 1:1
    ...
    %练习二 -- 部分报告法练习
    %声音线索随机化
    cuePosition = repmat([1,2,3],[1,trialsPart]);
